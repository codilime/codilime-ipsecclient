...
export const Endpoints = ({ data }) => {
    ...
    const dynamicHeader = tableHeaderSchema.map((el) => (
        <th key={el.item}>{el.item}</th>
    ));

    const dynamicCreateEndpoint = useMemo(
        () =>
            endpoint.map((el, index) => <EachEndpoint key={index} data={el} />),
        [endpoint]
    );

    const createNewEndpoint = open ? (
        <EachEndpoint data={emptyEndpointSchema} active disabled />
    ) : null;

    const newEndpointButton = open
        ? "Close a new endpoint"
        : "Add a new endpoint";
                ...
                <tbody className="table__body">
                    ...
                    <tr className="table__addBtn">
                        <td className="table__columnBtn">
                            <EndpointButton onClick={handleToggle}>
                                {newEndpointButton}
                            </EndpointButton>
                        </td>
                    </tr>
