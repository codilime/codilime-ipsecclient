...
#Packages
RUN apk add --no-cache nginx gettext supervisor curl sqlite tzdata strongswan jq tcpdump haproxy
...
COPY docker/haproxy.cfg /etc/haproxy
COPY docker/haproxy.ini /etc/supervisor.d/
